...
//  digitalWrite(LED_BUILTIN, HIGH); // turn off LED during signal analysis
  ...
  for(int s=1; s<rms_factor; s++)
  {
    
    if((baseline[s]-baseline[s-1])>slope_base)
    slope_base=baseline[s]-baseline[s-1];
  }
  slope_thresh=slope_thresh+slope_base;
  phase=2;
}
